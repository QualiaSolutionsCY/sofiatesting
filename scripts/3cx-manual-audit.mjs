...
// Telegram
const ZYPRESS_OTHERS_CHAT_ID = -1003337263793;
...
function normalizePhone(phone) {
  ...
  if (cleaned.startsWith('0') && cleaned.length === 9) return '+357' + cleaned.substring(1);
  if (cleaned.length === 8 && (cleaned.startsWith('2') || cleaned.startsWith('9') || cleaned.startsWith('7'))) return '+357' + cleaned;
...
function isInternal(number) {
  return INTERNAL_EXTENSIONS.includes(number) ||
...
async function main() {
  ...
  console.log(`Date: ${new Date().toLocaleDateString('en-CA', { timeZone: 'Europe/Nicosia' })}`);
  console.log('');
  ...
  try {
    // Navigate to 3CX
    await page.goto(CX3_URL, { waitUntil: 'networkidle', timeout: 30000 });
    ...
    if (!usernameField || !passwordField) {
      ...
      const inputs = await page.$$eval('input', els => els.map(e => ({
        type: e.type, name: e.name, id: e.id, placeholder: e.placeholder, className: e.className
      })));
    ...
    for (const path of callLogPaths) {
      try {
        await page.goto(CX3_URL + path, { waitUntil: 'networkidle', timeout: 15000 });
        await page.waitForTimeout(3000);
        const url = page.url();
        const content = await page.content();
        if (content.includes('call') || content.includes('Call') || content.includes('history')) {
          foundCallLog = true;
    ...
    if (!foundCallLog) {
      ...
      const navSelectors = [
        ...
        'text=Call History', '[data-testid*="calllog"]',
        'a[href*="call"]', 'button:has-text("Calls")',
      ];
    ...
    const tableRows = await page.$$eval('table tr, .call-row, .call-item, [class*="call"], [class*="row"]', rows => {
      return rows.map(row => ({
        text: row.innerText,
    ...
    const apiData = await page.evaluate(async ({ startOfDay, endOfDay, targetNumber }) => {
      const endpoints = [
        { url: `/api/calllog?dateFrom=${encodeURIComponent(startOfDay)}&dateTo=${encodeURIComponent(endOfDay)}&filter=${encodeURIComponent(targetNumber)}`, method: 'GET' },
      ...
      for (const ep of endpoints) {
        try {
          ...
          if (res.ok) {
            ...
            return { endpoint: ep.url, text: text.substring(0, 2000), status: res.status };
          }
        } catch (e) {
          // continue
        }
    ...
    if (apiData && apiData.data) {
      ...
      const list = Array.isArray(raw) ? raw : (raw.list || raw.calls || raw.data || []);
    ...
    for (const entry of entries) {
      ...
      const callerNumber = entry.callerNumber || entry.CallerNumber || entry.Caller ||
                           entry.from || entry.From || entry.caller || entry.source || entry.Source;
      const calledNumber = entry.calledNumber || entry.CalledNumber || entry.Called ||
                           entry.to || entry.To || entry.called || entry.destination || entry.Destination;
      const callTime = entry.callTime || entry.CallTime || entry.Time || entry.StartTime ||
                        entry.start_time || entry.timestamp || entry.DateTime;
      const direction = entry.direction || entry.Direction || entry.CallDirection ||
                         entry.call_direction || entry.Type || entry.type || '';
      ...
      if (callerNumber && calledNumber) {
        ...
        const isInbound = dir.includes('inbound') || dir === 'in' || dir === '1' || dir === '';
      ...
      if (entry.rawText && entry.phones) {
        for (const phone of entry.phones) {
          ...
          if (cleaned === TARGET_NUMBER || cleaned.length < 7 || isInternal(cleaned)) continue;
    ...
    for (const [phone, callTime] of externalCallers) {
      ...
      if (data && data.length > 0) {
        console.log(`  FOUND: ${formatPhoneDisplay(phone)} in ${data[0].group_name} (${data[0].message_date})`);
      } else {
        console.log(`  MISSING: ${formatPhoneDisplay(phone)} - not in any group`);
        missingCallers.push({ phone, callTime });
      }
    }
    ...
    if (missingCallers.length === 0) {
      ...
      for (const { phone, callTime } of missingCallers) {
        ...
        const timeDisplay = callTime ? new Date(callTime).toLocaleTimeString('en-GB', {
      ...
      // Also save to DB
      for (const { phone, callTime } of missingCallers) {
        await supabase.from('caller_alerts').insert({
          caller_phone: phone,
          status: 'alerted',
