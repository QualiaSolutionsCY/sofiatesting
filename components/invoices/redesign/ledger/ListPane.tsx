...
import { clientById, fmt, STAGES } from "@/lib/invoices/redesign/data";
...
const STAGE_OPTIONS: Array<{ value: Filters["stage"]; label: string }> = [
  ...
  {
    value: STAGES.SENT_TO_ACCOUNTING.id,
    label: STAGES.SENT_TO_ACCOUNTING.label,
  },
    ...
    return docs.filter((doc) => {
      ...
      if (filters.q) {
        ...
        const hay = [
          ...
          .join(" ")
          .toLowerCase();
        if (!hay.includes(q.replace(/[€,\s]/g, "")) && !hay.includes(q))
          return false;
      }
      return true;
    });
                  ...
                  <span className="row-meta">
                    ...
                    {doc.kind !== "credit"
                      ? (() => {
                          ...
                          if (paid)
                            return (
                              <span className="is-paid row-pay">Paid</span>
                            );
                          if (doc.stage === STAGES.APPROVED.id)
                            return (
                              <span className="is-approved row-pay">
                                Approved
                              </span>
                            );
                          return (
                            <span className="is-unpaid row-pay">Unpaid</span>
                          );
                        })()
                      : null}
