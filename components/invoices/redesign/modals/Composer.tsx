...
const plus30ISO = () =>
  toISODate(new Date(Date.now() + DUE_DAYS_DEFAULT * 86_400_000));
const currentPeriod = () =>
...
export function Composer({
  ...
  useEffect(() => {
    ...
    if (initial) {
      ...
      setVatMode(
        initial.vatMode === "included-vat"
          ? "included-vat"
          : initial.vatMode === "no-vat"
            ? "no-vat"
            : "plus-vat"
      );
  ...
  const dueDays =
    issued && due
      ? String(
          Math.max(
            0,
            Math.round((Date.parse(due) - Date.parse(issued)) / 86_400_000)
          )
        )
      : "";
  ...
  const clientMatches = (
    clientQuery
      ? CLIENTS.filter((c) => c.name.toLowerCase().startsWith(clientQuery))
      : CLIENTS
  ).slice(0, 6);
  ...
  const previewDoc: Doc = useMemo(
    () => ({
      ...
      appliesTo:
        kind === "credit"
          ? (invoices.find((i) => i.id === sourceInvoiceId)?.officialNo ??
            undefined)
          : undefined,
      commission: commission
        ? { agent, rate: "5%", amount: sub * 0.05 }
        : undefined,
    ...
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
        <div className="composer-header">
          <div>
            <p className="eyebrow">
              {isEdit ? "Edit document" : `New ${kindLabel}`}
            </p>
            <h2>
              {isEdit
                ? `Edit ${initial?.officialNo ? "№ " + initial.officialNo : initial?.draftNo}`
                : "Create a draft for Marios"}
            </h2>
            <p
              className="composer-doc-number"
              style={{ marginTop: 4, fontSize: ".8rem", color: "var(--muted)" }}
            >
              {initial?.officialNo
        ...
        <div className="composer-body">
          <div className="composer-form-pane">
            ...
            <div className="composer-grid">
              ...
              >
                ...
                <input
                  autoComplete="off"
                  onBlur={() =>
                    window.setTimeout(() => setClientListOpen(false), 150)
                  }
                  onChange={(event) => {
                  ...
                  onFocus={() =>
                    setClientListOpen(newClient.name.trim().length > 0)
                  }
              ...
              <label>
                ...
                {isSourceMode ? (
                  <select
                    onChange={(event) =>
                      selectInvoiceForSource(event.target.value)
                    }
                    required
                  ...
                  >
                    ...
                    {sourceInvoices.map((inv) => (
                      <option key={inv.id} value={inv.id}>
                        № {inv.officialNo} · {fmt(Math.abs(inv.total))} ·{" "}
                        {inv.issued}
                      </option>
                    ))}
                  </select>
                ...
                <label>
                  ...
                  >
                    ...
                    <option value="included-vat">
                      Including VAT — already in price
                    </option>
            ...
            {isSourceMode ? null : (
              <div className="lines-editor">
                <div className="lines-editor-head">
                  <h3>Lines</h3>
                  <span>
                    {lines.length} item{lines.length === 1 ? "" : "s"} · VAT{" "}
                    {rate}%
                  </span>
                </div>
              ...
              <div className="optional-composer-grid composer-grid">
                ...
                {recurrence !== "none" ? (
                  <label style={{ gridColumn: "1 / -1" }}>
                    ...
                    <input
                      onChange={(event) =>
                        setRecurrenceEmail(event.target.value)
                      }
                ...
                <label>
                  ...
                  <select
                    onChange={(event) =>
                      setCommission(event.target.value === "yes")
                    }
                    value={commission ? "yes" : "no"}
                  >
                ...
                {commission ? (
                  <label style={{ gridColumn: "1 / -1" }}>
                    ...
                    <input
                      ...
                      placeholder="e.g. Christos Lambrou"
                      value={agent}
                    />
                  </label>
                ) : null}
            ...
            <p className="composer-description">
              ...
              {isEdit ? (
                <>
                  Changes will be saved to the existing document. Sophia will
                  note the edit in the audit trail.
                </>
              ) : isSourceMode ? (
                <>
                  ...
                  and send it to the accounting group, mirroring that
                  invoice&apos;s amount and VAT.
                </>
              ) : (
                <>
                  Sophia will prepare a draft with a fake number, route it to
                  Marios via WhatsApp{" "}
                  <strong style={{ color: "var(--ink)" }}>CSC Review</strong>,
                  and only apply an official number after Marios approves.
                </>
              )}
            </p>
          </div>
