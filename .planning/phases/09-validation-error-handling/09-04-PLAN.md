...
must_haves:
  truths:
    - "Users receive immediate feedback when images are invalid"
    - "Validation errors explain how to fix the issue (e.g., send from gallery)"
    - "Debugging is possible via correlation ID linking images to original request"
  artifacts:
...
Note: The `correlation_id` column may not exist yet. The code MUST handle this gracefully.

**CRITICAL: Add backward-compatibility wrapper** to ensure code works whether or not the column exists:

In `addPendingImages`, wrap the correlation_id usage:
```typescript
const records = imageUrls.map(url => {
...
In `getPendingImages`, keep the select simple to avoid errors:
```typescript
const { data, error } = await supabase
  .from("pending_images")
  .select("image_url")  // Keep simple, correlation_id is for future debug queries
  .eq("phone_number", phoneNumber)
  .order("created_at", { ascending: true });
```

This ensures the code works before AND after the migration is applied.
  </action>
  <verify>
```bash
# Verify correlation tracking added
grep -n "correlation" supabase/functions/sophia-bot/services/pending-images.ts
# Verify backward-compatibility wrapper exists
grep -n "Record<string, unknown>" supabase/functions/sophia-bot/services/pending-images.ts
```
  </verify>
  <done>Pending images track correlation ID with backward-compatible schema handling</done>
</task>
