...
async function main() {
  ...
  console.log(
    "Testing: Paphos listing should be assigned to Lauren (listings@zyprus.com)\n"
  );
      ...
      relationships: {
        ...
        field_property_type: {
          data: {
            ...
            id: "e3c4bd56-f8c4-4672-b4a2-23d6afe6ca44",
          },
        },
        field_listing_type: {
          data: {
  ...
  const reviewerRel =
    verifyData.data.relationships?.field_ai_listing_reviewer?.data;
  const instructorRel =
    verifyData.data.relationships?.field_ai_listing_instructor?.data;

  console.log("\nReviewer relationship:", JSON.stringify(reviewerRel, null, 2));
  console.log(
    "Instructor relationship:",
    JSON.stringify(instructorRel, null, 2)
  );

  // Check included users
  const includedUsers =
    verifyData.included?.filter((i: any) => i.type === "user--user") || [];
  console.log("\nIncluded users:");
  for (const user of includedUsers) {
    console.log(
      `  - ${user.attributes?.display_name || user.attributes?.name}: ${user.id}`
    );
  }

  // Verify
  const reviewerUuid = Array.isArray(reviewerRel)
    ? reviewerRel[0]?.id
    : reviewerRel?.id;
  ...
  console.log(
    `   https://dev9.zyprus.com/node/${result.data.attributes.drupal_internal__nid}/edit`
  );
