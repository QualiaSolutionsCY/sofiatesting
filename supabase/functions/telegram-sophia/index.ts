...
// SOPHIA Kill Switch - set to "false" to disable bot responses
const SOPHIA_ENABLED = Deno.env.get("SOPHIA_TELEGRAM_ENABLED") !== "false";
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Kill switch - silently ignore messages when disabled
    if (!SOPHIA_ENABLED) {
      console.log("[Webhook] SOPHIA disabled, ignoring message");
      return jsonResponse({ ok: true, reason: "sophia_disabled" });
    }
...
const handlePhoneRegistration = async (
  ...
  if (!success) {
    ...
    captureMessage("Agent registration failed", "error", {
      ...
      extra: { agent_id: agent.id, user_id: userId },
    });
    return;
  }
...
const callOpenRouter = async (
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        // Capture API errors to Sentry
        if (response.status >= 500) {
          captureMessage(`OpenRouter API error: ${response.status}`, "error", {
            tags: { api: "openrouter", status: String(response.status) },
            extra: { error_text: errorText.substring(0, 500) },
          });
        }
        continue;
      }
  ...
  // All retries failed - capture to Sentry
  if (lastError) {
    captureException(lastError, {
      transaction: "callOpenRouter",
      tags: { api: "openrouter", retries_exhausted: "true" },
    });
  }
