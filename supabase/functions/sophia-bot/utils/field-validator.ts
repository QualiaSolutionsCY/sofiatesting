...
export function isRequestingInformation(content: string): boolean {
  ...
  if (isCompletedViewingForm(content)) {
    logger.debug(
      "[Field Validator] Detected completed viewing form, not requesting info",
  ...
  if (isCompletedMarketingAgreement(content)) {
    logger.debug(
      "[Field Validator] Detected completed marketing agreement, not requesting info",
      { category: LogCategory.GENERAL }
    );
    return false;
  }
  ...
  if (isCompletedReservationAgreement(content)) {
    logger.debug(
      "[Field Validator] Detected completed reservation agreement, not requesting info",
      { category: LogCategory.GENERAL }
    );
    return false;
  }
  ...
  if (bulletListCount >= 3 && content.length < 800) {
    ...
    logger.debug(
      "[Field Validator] Bullet list requesting info (count: " +
        bulletListCount +
        ")",
      { category: LogCategory.GENERAL }
    );
...
function validateViewingFormFields(content: string): boolean {
  ...
  if (isCompletedViewingForm(content)) {
    logger.debug(
      "[Field Validator] Viewing form passes completed document check",
      { category: LogCategory.GENERAL }
    );
    return true;
  }
  ...
  if (missingFields > 0) {
    logger.debug(
      `[Field Validator] Viewing form missing ${missingFields} fields: ${missingFieldNames.join(", ")}`,
      { category: LogCategory.GENERAL }
    );
  }
...
function validateReservationAgreementFields(content: string): boolean {
  ...
  if (isCompletedReservationAgreement(content)) {
    logger.debug(
      "[Field Validator] Reservation agreement passes completed document check",
      { category: LogCategory.GENERAL }
    );
    return true;
  }
  ...
  if (hasBlankBrackets || hasRepeatedDots || hasUnderscoreLines) {
    ...
    if (hasDocStructure) {
      logger.debug(
        "[Field Validator] Blank reservation agreement detected - allowing blank document",
        { category: LogCategory.GENERAL }
      );
      return true;
    }
  ...
  if (missingFields > 0) {
    logger.debug(
      `[Field Validator] Reservation agreement missing ${missingFields} fields: ${missingFieldNames.join(", ")}`,
      { category: LogCategory.GENERAL }
    );
  }
...
export function hasAllRequiredFields(
  ...
  if (isCompletedViewingForm(aiResponse)) {
    logger.debug(
      "[Field Validator] hasAllRequiredFields: Detected completed viewing form -> PASS",
      { category: LogCategory.GENERAL }
    );
    return true;
  }
  ...
  if (isCompletedMarketingAgreement(aiResponse)) {
    logger.debug(
      "[Field Validator] hasAllRequiredFields: Detected completed marketing agreement -> PASS",
      { category: LogCategory.GENERAL }
    );
    return true;
  }
  ...
  if (isCompletedReservationAgreement(aiResponse)) {
    logger.debug(
      "[Field Validator] hasAllRequiredFields: Detected completed reservation agreement -> PASS",
      { category: LogCategory.GENERAL }
    );
    return true;
  }
  ...
  if (templateType) {
    ...
    if (
      templateType.includes("reservation-agreement") ||
      templateType.includes("reservation agreement")
    ) {
  ...
  if (
    ...
    lowerResponse.includes("property viewing")
  ) {
    return validateViewingFormFields(aiResponse);
  }
  if (
    lowerResponse.includes("reservation agreement") ||
    lowerResponse.includes("property reservation")
  ) {
    return validateReservationAgreementFields(aiResponse);
  }
