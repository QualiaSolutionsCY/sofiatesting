...
**Phone Number:** ${context.phoneNumber}
${context.agentEmail ? `**Email:** ${context.agentEmail}
` : ''}
...
`;
  ...
  if (context.lastDocument) {
    const docTypeDisplay = context.lastDocument.document_type?.replace(/_/g, ' ') || 'document';
    documentContext = `
...
---
`;
    logger.info(`[DocContext] Found available document for attachment: ${context.lastDocument.document_name}`, { category: LogCategory.GENERAL });
  }
  ...
  return baseSystemPrompt + dateContext + senderContext + imageContext + documentContext + (context.personalizationContext || "");
}
...
export async function chat(
  ...
  if (!OPENROUTER_API_KEY) {
    logger.error("CRITICAL: OPENROUTER_API_KEY is not set", undefined, { category: LogCategory.GENERAL });
    return { response: "Service configuration error. Please contact support.", success: false };
  }

  // Convert Gemini history format to OpenRouter format
  const openrouterMessages: Array<{role: string, content: string, tool_calls?: any[], tool_call_id?: string}> = [
  ...
  while (toolCallCount < maxToolCalls) {
    ...
    if (error || !message) {
      return {
        response: "I'm experiencing technical difficulties right now. Please try again in a few moments.",
        success: false
      };
    }
    ...
    if (message?.tool_calls && message.tool_calls.length > 0) {
      ...
      for (const toolCall of message.tool_calls) {
        ...
        try {
          ...
          logger.error(`Tool error: Failed to parse arguments for ${toolName}`, undefined, { category: LogCategory.TOOL });
          toolArgs = {};
        }
        ...
        if (toolResult.success && toolResult.message) {
          ...
          if (toolName === "createPropertyListing" || toolName === "createLandListing") {
            ...
            logger.info(`[Images] Cleared pending images for ${cleanPhone} after successful upload`, { category: LogCategory.IMAGE });
          }
  ...
  if (!aiResponse) {
    ...
    return {
      response: "I couldn't generate a response. Please rephrase your request and try again.",
      success: false
    };
  }
