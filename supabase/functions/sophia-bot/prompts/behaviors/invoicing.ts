...
3. When Marios approves, do NOT ask ANYTHING. You ALREADY collected the agent name(s) when the draft was created — reuse them silently. Call approve IMMEDIATELY with \`groupMessage\` set to just the name(s) you already have (e.g. "Christos", or "Christos, Maria"). NEVER re-ask "which agent made the sale?" at approval, and NEVER say "shall I use {name}?" — you already have the name, so just approve. The only time you ask at approval is if the agent name was genuinely never given in this whole conversation.
