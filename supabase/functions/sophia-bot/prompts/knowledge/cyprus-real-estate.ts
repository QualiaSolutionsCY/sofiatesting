 ...
 *
 * SOURCE: DB key 'cyprus_knowledge' (this file is FALLBACK only)
 *
 * This file is used ONLY when:
 * 1. Database is unavailable
 * 2. The 'cyprus_knowledge' key is missing from sophia_prompts table
 *
 * To edit in production:
 * 1. Edit in Supabase Dashboard: sophia_prompts WHERE key = 'cyprus_knowledge'
 * 2. POST to /admin/prompts/invalidate to clear cache
 *
 * DO NOT edit this file for production changes - edit the database instead.
 *
 * Comprehensive knowledge for answering questions
 */

export const CYPRUS_KNOWLEDGE = `## Cyprus Real Estate Knowledge Base
...
- Option A: Connect bank/fund directly with client, be included in email CC
- Option B: Request details from buyer's lawyer (bypasses bank bureaucracy)
- Option C: Ask client to provide information directly
...
| Unit Type | Core City Center (m²) | Other Areas (m²) |
|-----------|------------------|------------------|
| Studio | 30 | 35 |
| 1 Bedroom | 45 | 50 |
| 2 Bedroom | 65 | 75 |
...
- Core City Center properties have stricter requirements than outer areas
...
- Eligibility: No domicile of origin in Cyprus, obtained domicile of choice outside Cyprus, NOT Cyprus tax resident for 20+ consecutive years before the tax year
- Benefits: 17-year exemption from special defense contribution on dividends and interest
...
Caps: Monthly EUR 5,239, Annual EUR 62,868
...
50% discount usually applies.

**VAT (New Properties):**
- Standard rate: 19%
- Reduced rate: 5% (for primary residence)
...
- Rate: 0.4% of selling price
- Example: EUR 180,000 sale × 0.4% = EUR 720
...
### 8. Planning Zones & Building Density

**FIELD COLLECTION - When user asks about planning zone, zoning, or what to build on a plot:**

Ask for these 4 fields in ONE message:
"I'll help you calculate the building potential. Please provide:

**Plot size** (e.g., 530m²)
**Building density** (e.g., 100% or 120%)
**Site coverage** (e.g., 50%)
**Number of floors** (e.g., 3 floors)"
...
**RESPONSE FORMAT FOR PLANNING CALCULATIONS:**

1. Start with bold disclaimer: "**The following calculations are based on the assumption that this is already a Plot and not a land parcel.**"

2. Show calculation table with these rows (USE CORRECT TERMINOLOGY):
   - **Gross Buildable Area** = Plot × Density (total floor area allowed)
   - **Max Ground Coverage** = Plot × Coverage %
   - **Max Floors** = as provided
   - **Net Buildable Area** = Gross Buildable - 10% (available for apartments after communal areas)
   - **Covered Verandas** = 25% of Gross Buildable Area (for Local Town Plans)

3. Building Strategy section - MUST include veranda fit check:
   - Calculate: Gross Buildable Area ÷ Number of Floors = Gross per floor
   - Calculate: Covered Verandas ÷ Number of Floors = Verandas per floor
   - Show: Gross per floor + Verandas per floor = Total footprint per floor
   - Confirm this fits within Max Ground Coverage

4. Example of the layout of the building - USE 2 BEDROOM APARTMENTS ONLY:
   - Use minimum 75m² per 2-bed apartment (Other Areas minimum)
   - Reasonable range: 75-85m² per unit
   - Calculate number of units: Net Buildable Area ÷ 75m²
   - Calculate: Covered Verandas ÷ Number of Units = Veranda per unit
   - Show final unit size as: Apartment m² + Veranda m² = Total per unit

5. End with warning: "**Important:** This is indicative only. Please consult a qualified architect for detailed planning."

6. DO NOT ask follow-up questions at the end. No "Would you like me to calculate..." etc.

**TERMINOLOGY - VERY IMPORTANT:**
- **Gross Buildable Area** = Plot × Density (this is the TOTAL allowed)
- **Net Buildable Area** = Gross - 10% (this is what's available for apartments)
- **Gross Buildable per Floor** = Gross Buildable ÷ Floors (NOT "Net Indoor per Floor")
- ALWAYS deduct 10% for communal areas to get Net Buildable Area

**Calculation Example:**
Given: 540 m² plot, Density 1.2 (120%), Coverage 0.5 (50%), 3 floors

**The following calculations are based on the assumption that this is already a Plot and not a land parcel.**

| Parameter | Calculation | Result |
|-----------|-------------|--------|
| Gross Buildable Area | 540 m² × 1.20 | 648 m² |
| Max Ground Coverage | 540 m² × 0.50 | 270 m² |
| Max Floors | Provided | 3 Floors |
| Net Buildable Area | 648 m² - 10% (communal) | ~583 m² |
| Gross Buildable per Floor | 648 m² ÷ 3 | 216 m² |
| Covered Verandas | 25% of 648 m² | 162 m² |
| Verandas per Floor | 162 m² ÷ 3 | 54 m² |

**Building Strategy:**
- Gross Buildable per floor: 216 m²
- Verandas per floor: 54 m²
- Total footprint per floor: 216 m² + 54 m² = 270 m² (fits exactly within the 270 m² coverage limit)

**Example of the layout of the building (2 Bedroom Apartments):**
- Net Buildable Area: 583 m²
...
**Minimum Apartment Sizes (Other Areas - Not Core City Center):**
| Unit Type | Minimum (m²) |
|-----------|--------------|
| Studio | 35 |
| 1 Bedroom | 50 |
...
- Corridors
- Covered verandas (if exceeding limits)
...
|-----------|------------------|
| Local Town Plans (Τοπικά Σχέδια) | 25% of buildable area |
| Rural Countryside Plan (Δήλωση Πολιτικής) | 20% of buildable area |
